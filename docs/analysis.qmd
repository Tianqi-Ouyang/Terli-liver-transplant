...
|:--|:--|:--|
| S1 | Shared `create_table_one()` passed the Overall column into `pvalue()` → HTML p-values inflated | **Fixed in the shared pipeline 2026-09-15** (tableone only); this file uses the fixed function |
| S2 | Shared pipeline yielded `mean_map_day0.x` / `.y` instead of `mean_map_day0` | **Fixed in the shared pipeline 2026-09-15**; `map_day0_avg` is still computed here because it also handles S3 (`0 → NA`) |
...
Tables use the shared pipeline's `create_table_one()` (fixed 2026-09-15 —
tableone only: `tableone::CreateTableOne()` → `print()` → `write.csv()`, and
the same printed matrix is returned as a `knitr::kable()`), so the web table
and the CSV are identical. Conventions (tableone defaults, matching every
earlier Table 1 CSV in the registry): continuous variables as median [IQR]
with the Kruskal-Wallis rank-sum test (`test = nonnorm`; equivalent to
Mann-Whitney for two groups); categorical variables as n (%) with the
chi-squared test (continuity-corrected for 2 × 2). `Missing` is the percentage
of the cohort with a missing value.
