...
The legacy `Code/*.Rmd` scripts, the shared pipeline and the EFU site files
were reviewed before this analysis was written. All findings, with their
status in the other HARMONY sub-projects, are listed on the
[**Bug Tracker**](bugs.qmd) page. The ones that change how this file works:

| ID | Bug | Handling in this file |
|:--|:--|:--|
| S1 | Shared `create_table_one()` passes the Overall column into `pvalue()` → HTML p-values inflated | Not used: tables are built with `tableone::CreateTableOne()` only and the printed matrix is shown on the page (see *Table Helper*) |
| S2 | Shared pipeline yields `mean_map_day0.x` / `.y` instead of `mean_map_day0` | `map_day0_avg` computed here |
| S3 | Day-0 MAP fields use `0` as "not measured" | `0 → NA` before averaging |
| L5 | Old scripts used serum albumin (`alb_terli_dayN`) for "albumin given" | `albumintotal_terli_dayX` row = shared `total_albumin` (grams given, days 0–13) |
| D1 | Yale IDs `YAL-00N` vs `Yale-N` | Normalized join key |
| D2 | One CCF EFU row is not in the master | Cohort n = 221 (computed) |
...
# Table Helper (tableone only)

The shared pipeline's `create_table_one()` renders an HTML table with the
`table1` package whose p-values are wrong (bug **S1**: the Overall column is
tested as a third group). This project does not use that path. The function
below is the original, tableone-only version: `tableone::CreateTableOne()`
→ `print()` → `write.csv()`, and the **same printed matrix** is shown on this
page with `knitr::kable()`, so the web table and the CSV are identical.

Conventions (tableone defaults, matching every earlier Table 1 CSV in the
registry): continuous variables as median [IQR] with the Kruskal-Wallis
...
```{r table-one-helper}
#| code-summary: "Show create_table_one() (tableone only)"
# Overrides the shared-pipeline version for this document (chunk environment
# takes precedence). Returns the printed tableone matrix invisibly so the
# caller can kable() exactly what was written to the CSV.
...
knitr::kable(t1, caption = "Table 1 - Demographics by efu_listingstatus (0 = never listed, 1 = listed at any point)")
```
...
knitr::kable(t2, caption = "Table 2 - EFU outcomes by listed_transplant (0 = not listed during index hospitalization, 1 = listed)")
```
...
knitr::kable(t3, caption = "Table 3 - EFU outcomes by efu_listingstatus (0 = never listed, 1 = listed at any point)")
```
